view: top_n_nations {
  derived_table: {
    explore_source: suppliers {
      ...
      derived_column: ranking {
        sql: rank() over (order by {% parameter nation_rank_measure_selection %} desc , count_suppliers desc) ;;
      }
  ...
  dimension: nation_name_top_nation {
    ...
    sql:
          CASE
            WHEN ${nation_rank}<{% parameter top_rank_limit %} THEN ${nation_title_handle}
            ELSE 'Other'
          END
    ;;
  }
